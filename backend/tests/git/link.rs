...
mod delete_link {
    ...
    fn link_deleted() {
        ...
        assert!(f.git.exists(repo_id, link_id).unwrap());
    ...
    fn activity_log_updated() {
        ...
        let activity = f.git.fetch_activity(repo_id, link_id, 1).unwrap();
        ...
        let activity = f.git.fetch_activity(repo_id, link_id, 100).unwrap();
        let mut found = false;
...
mod update_link_parent_topics {
    ...
    fn topics_updated() {
        ...
        let link_id = link.id();
        let link = f.git.fetch_link(repo_id, link_id).unwrap();
...
mod upsert_link {
    ...
    fn link_added() {
        ...
        assert!(!f.git.exists(repo_id, &link_id).unwrap());
        assert!(!f.git.appears_in(repo_id, &search, &entry).unwrap());
    ...
    fn no_orphans() {
        ...
        f.upsert_link(repo_id, &url, None, None);
    ...
    fn updates_are_idempotent() {
        ...
        f.upsert_link(repo_id, &url, None, Some(parent_topic.to_owned()));
        let UpsertLinkResult { alerts, .. } =
            f.upsert_link(repo_id, &url, None, Some(parent_topic));
    ...
    fn details_updated() {
        ...
        assert!(!f.git.exists(repo_id, &path).unwrap());

        f.upsert_link(repo_id, &url, Some("A".into()), Some(parse_id("00001")));

        f.fetch_link(repo_id, &path, |link| {
        ...
        f.upsert_link(repo_id, &url, Some("B".into()), Some(parse_id("00002")));
    ...
    fn parent_topic_updated() {
        ...
        let result = f.upsert_link(repo_id, &url, Some("A".into()), Some(topic.to_owned()));
        let link = result.link.unwrap();

        f.fetch_topic(repo_id, &topic, |parent| {
    ...
    fn lookup_indexes_updated() {
        ...
        assert!(f
            .git
            .appears_in(repo_id, &search, &link.unwrap().to_search_entry())
            .unwrap());

        let UpsertLinkResult { link, .. } =
            f.upsert_link(repo_id, &url, Some("a url title".into()), Some(topic));
        assert!(!f
            .git
            .appears_in(repo_id, &search, &link.unwrap().to_search_entry())
            .unwrap());
    ...
    fn link_added_to_correct_repo() {
        ...
        let url = valid_url();
        let repo_id = RepoId::wiki();
        let other_repo_id = RepoId::other();
        let link_id = url.id().unwrap();
        let parent_id = parse_id("00001");

        assert!(!f.git.exists(other_repo_id, &link_id).unwrap());
        ...
        let topic = f.git.fetch_topic(repo_id, &parent_id).expect("00001");
