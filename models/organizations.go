...
type organizationR struct {
	Links        LinkSlice
	Topics       TopicSlice
	TopicsLinks  TopicsLinkSlice
	TopicsTopics TopicsTopicSlice
}
...
// TopicsLinks retrieves all the topics_link's TopicsLinks with an executor.
func (o *Organization) TopicsLinks(mods ...qm.QueryMod) topicsLinkQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"topics_links\".\"organization_id\"=?", o.ID),
	)
...
// TopicsTopics retrieves all the topics_topic's TopicsTopics with an executor.
func (o *Organization) TopicsTopics(mods ...qm.QueryMod) topicsTopicQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"topics_topics\".\"organization_id\"=?", o.ID),
	)
...
// LoadTopicsLinks allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (organizationL) LoadTopicsLinks(ctx context.Context, e boil.ContextExecutor, singular bool, maybeOrganization interface{}, mods queries.Applicator) error {
	var slice []*Organization
	var object *Organization
	...
	query := NewQuery(qm.From(`topics_links`), qm.WhereIn(`organization_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.TopicsLinks = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.OrganizationID {
				local.R.TopicsLinks = append(local.R.TopicsLinks, foreign)
				if foreign.R == nil {
					foreign.R = &topicsLinkR{}
				}
				foreign.R.Organization = local
...
// LoadTopicsTopics allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (organizationL) LoadTopicsTopics(ctx context.Context, e boil.ContextExecutor, singular bool, maybeOrganization interface{}, mods queries.Applicator) error {
	var slice []*Organization
	var object *Organization
	...
	query := NewQuery(qm.From(`topics_topics`), qm.WhereIn(`organization_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.TopicsTopics = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.OrganizationID {
				local.R.TopicsTopics = append(local.R.TopicsTopics, foreign)
				if foreign.R == nil {
					foreign.R = &topicsTopicR{}
				}
				foreign.R.Organization = local
...
// AddTopicsLinks adds the given related objects to the existing relationships
// of the organization, optionally inserting them as new records.
// Appends related to o.R.TopicsLinks.
// Sets related.R.Organization appropriately.
func (o *Organization) AddTopicsLinks(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*TopicsLink) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.OrganizationID = o.ID
	...
	if o.R == nil {
		...
		o.R.TopicsLinks = append(o.R.TopicsLinks, related...)
	}
...
// AddTopicsTopics adds the given related objects to the existing relationships
// of the organization, optionally inserting them as new records.
// Appends related to o.R.TopicsTopics.
// Sets related.R.Organization appropriately.
func (o *Organization) AddTopicsTopics(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*TopicsTopic) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.OrganizationID = o.ID
	...
	if o.R == nil {
		o.R = &organizationR{
			TopicsTopics: related,
		}
	} else {
		o.R.TopicsTopics = append(o.R.TopicsTopics, related...)
	}
