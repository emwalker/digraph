...
var TopicRels = struct {
	Organization       string
	ParentTopicsLinks  string
	ParentTopicsTopics string
	ChildTopicsTopics  string
}{
	Organization:       "Organization",
	ParentTopicsLinks:  "ParentTopicsLinks",
	ParentTopicsTopics: "ParentTopicsTopics",
	ChildTopicsTopics:  "ChildTopicsTopics",
}

// topicR is where relationships are stored.
type topicR struct {
	Organization       *Organization
	ParentTopicsLinks  TopicsLinkSlice
	ParentTopicsTopics TopicsTopicSlice
	ChildTopicsTopics  TopicsTopicSlice
}
...
// ParentTopicsLinks retrieves all the topics_link's TopicsLinks with an executor via parent_id column.
func (o *Topic) ParentTopicsLinks(mods ...qm.QueryMod) topicsLinkQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"topics_links\".\"parent_id\"=?", o.ID),
	)

	query := TopicsLinks(queryMods...)
	queries.SetFrom(query.Query, "\"topics_links\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"topics_links\".*"})
	}

	return query
}

// ParentTopicsTopics retrieves all the topics_topic's TopicsTopics with an executor via parent_id column.
func (o *Topic) ParentTopicsTopics(mods ...qm.QueryMod) topicsTopicQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"topics_topics\".\"parent_id\"=?", o.ID),
	)

	query := TopicsTopics(queryMods...)
	queries.SetFrom(query.Query, "\"topics_topics\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"topics_topics\".*"})
	}

	return query
}

// ChildTopicsTopics retrieves all the topics_topic's TopicsTopics with an executor via child_id column.
func (o *Topic) ChildTopicsTopics(mods ...qm.QueryMod) topicsTopicQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"topics_topics\".\"child_id\"=?", o.ID),
	)
...
// LoadParentTopicsLinks allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (topicL) LoadParentTopicsLinks(ctx context.Context, e boil.ContextExecutor, singular bool, maybeTopic interface{}, mods queries.Applicator) error {
	var slice []*Topic
	var object *Topic
	...
	query := NewQuery(qm.From(`topics_links`), qm.WhereIn(`parent_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load topics_links")
	}

	var resultSlice []*TopicsLink
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice topics_links")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on topics_links")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for topics_links")
	}
	...
	if singular {
		object.R.ParentTopicsLinks = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &topicsLinkR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.ParentID {
				local.R.ParentTopicsLinks = append(local.R.ParentTopicsLinks, foreign)
				if foreign.R == nil {
					foreign.R = &topicsLinkR{}
				}
...
// LoadParentTopicsTopics allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (topicL) LoadParentTopicsTopics(ctx context.Context, e boil.ContextExecutor, singular bool, maybeTopic interface{}, mods queries.Applicator) error {
	var slice []*Topic
	var object *Topic
	...
	query := NewQuery(qm.From(`topics_topics`), qm.WhereIn(`parent_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load topics_topics")
	}

	var resultSlice []*TopicsTopic
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice topics_topics")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on topics_topics")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for topics_topics")
	}
	...
	if singular {
		object.R.ParentTopicsTopics = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &topicsTopicR{}
			}
			foreign.R.Parent = object
		}
		return nil
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.ParentID {
				local.R.ParentTopicsTopics = append(local.R.ParentTopicsTopics, foreign)
				if foreign.R == nil {
					foreign.R = &topicsTopicR{}
				}
				foreign.R.Parent = local
...
// LoadChildTopicsTopics allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (topicL) LoadChildTopicsTopics(ctx context.Context, e boil.ContextExecutor, singular bool, maybeTopic interface{}, mods queries.Applicator) error {
	var slice []*Topic
	var object *Topic
	...
	query := NewQuery(qm.From(`topics_topics`), qm.WhereIn(`child_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.ChildTopicsTopics = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &topicsTopicR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.ChildID {
				local.R.ChildTopicsTopics = append(local.R.ChildTopicsTopics, foreign)
				if foreign.R == nil {
					foreign.R = &topicsTopicR{}
				}
...
// AddParentTopicsLinks adds the given related objects to the existing relationships
// of the topic, optionally inserting them as new records.
// Appends related to o.R.ParentTopicsLinks.
// Sets related.R.Parent appropriately.
func (o *Topic) AddParentTopicsLinks(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*TopicsLink) error {
	var err error
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.ID, rel.OrganizationID, rel.ParentID, rel.ChildID}
	...
	if o.R == nil {
		o.R = &topicR{
			ParentTopicsLinks: related,
		}
	} else {
		o.R.ParentTopicsLinks = append(o.R.ParentTopicsLinks, related...)
	}
...
// AddParentTopicsTopics adds the given related objects to the existing relationships
// of the topic, optionally inserting them as new records.
// Appends related to o.R.ParentTopicsTopics.
// Sets related.R.Parent appropriately.
func (o *Topic) AddParentTopicsTopics(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*TopicsTopic) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.ParentID = o.ID
			...
			values := []interface{}{o.ID, rel.ParentID, rel.ChildID}
	...
	if o.R == nil {
		o.R = &topicR{
			ParentTopicsTopics: related,
		}
	} else {
		o.R.ParentTopicsTopics = append(o.R.ParentTopicsTopics, related...)
	}
...
// AddChildTopicsTopics adds the given related objects to the existing relationships
// of the topic, optionally inserting them as new records.
// Appends related to o.R.ChildTopicsTopics.
// Sets related.R.Child appropriately.
func (o *Topic) AddChildTopicsTopics(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*TopicsTopic) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.ChildID = o.ID
	...
	if o.R == nil {
		o.R = &topicR{
			ChildTopicsTopics: related,
		}
	} else {
		o.R.ChildTopicsTopics = append(o.R.ChildTopicsTopics, related...)
	}
