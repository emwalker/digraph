...
func testOrganizationToManyTopicsLinks(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	topicsLink, err := a.TopicsLinks().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := OrganizationSlice{&a}
	if err = a.L.LoadTopicsLinks(ctx, tx, false, (*[]*Organization)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.TopicsLinks); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.TopicsLinks = nil
	if err = a.L.LoadTopicsLinks(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.TopicsLinks); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testOrganizationToManyTopicsTopics(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	topicsTopic, err := a.TopicsTopics().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := OrganizationSlice{&a}
	if err = a.L.LoadTopicsTopics(ctx, tx, false, (*[]*Organization)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.TopicsTopics); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.TopicsTopics = nil
	if err = a.L.LoadTopicsTopics(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.TopicsTopics); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testOrganizationToManyAddOpTopicsLinks(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddTopicsLinks(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.TopicsLinks[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.TopicsLinks[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.TopicsLinks().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testOrganizationToManyAddOpTopicsTopics(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddTopicsTopics(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.TopicsTopics[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.TopicsTopics[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.TopicsTopics().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
