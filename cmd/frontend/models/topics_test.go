...
func testTopicToManySynonyms(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, synonymDBTypes, false, synonymColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, synonymDBTypes, false, synonymColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.Synonyms().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := TopicSlice{&a}
	if err = a.L.LoadSynonyms(ctx, tx, false, (*[]*Topic)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Synonyms); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.Synonyms = nil
	if err = a.L.LoadSynonyms(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Synonyms); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testTopicToManyAddOpSynonyms(t *testing.T) {
	var err error
	...
	foreigners := []*Synonym{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, synonymDBTypes, false, strmangle.SetComplement(synonymPrimaryKeyColumns, synonymColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*Synonym{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddSynonyms(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.Synonyms[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.Synonyms[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.Synonyms().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
