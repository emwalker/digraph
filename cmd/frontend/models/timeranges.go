...
var (
	...
	timerangeColumnsWithoutDefault = []string{"starts_at", "finishes_at"}
	timerangeColumnsWithDefault    = []string{"id", "prefix_format"}
	timerangePrimaryKeyColumns     = []string{"id"}
)

type (
	// TimerangeSlice is an alias for a slice of pointers to Timerange.
	// This should generally be used opposed to []Timerange.
...
func (timerangeL) LoadTopics(ctx context.Context, e boil.ContextExecutor, singular bool, maybeTimerange interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(qm.From(`topics`), qm.WhereIn(`topics.timerange_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
func Timeranges(mods ...qm.QueryMod) timerangeQuery {
	...
	return timerangeQuery{NewQuery(mods...)}
}
